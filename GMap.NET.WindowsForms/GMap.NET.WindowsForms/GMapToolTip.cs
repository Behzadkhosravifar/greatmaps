...
#endif
    {
        protected Point? _lastDragPos { get; set; }
...
#if !PocketPC
        public static readonly Font DefaultFont = new Font("B Nazanin", 14, FontStyle.Bold, GraphicsUnit.Pixel);
#else
      ...
      public static readonly Brush DefaultFill = new System.Drawing.SolidBrush(Color.AliceBlue);
        ...
        public void OnMouseDrag(Graphics g, System.Windows.Forms.MouseEventArgs e)
        {
            if (!_lastDragPos.HasValue)
                return;

            Offset = new Point(Offset.X + e.X - _lastDragPos.Value.X, Offset.Y + e.Y - _lastDragPos.Value.Y);

            _lastDragPos = e.Location;
        }

        public void StartMouseDrag(System.Windows.Forms.MouseEventArgs e)
        {
            _lastDragPos = e.Location;
        }
        ...
        public bool IsMouseOver(Graphics g, Point mousePos, GPoint markerPos)
        {
            var st = g.MeasureString(Marker.ToolTipText, Font).ToSize();

            var rect = new Rectangle((int)markerPos.X + Offset.X, (int)markerPos.Y + Offset.Y - st.Height, st.Width + TextPadding.Width * 2, st.Height + TextPadding.Height);

            var res = mousePos.X >= rect.X
